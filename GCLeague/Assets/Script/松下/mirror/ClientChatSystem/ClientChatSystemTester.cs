...
using MirrorChatSystems; // mirrorのチャットシステムを利用


public class ClientChatSystemTester : NetworkBehaviour
{
    public MirrorNetWorkManage m_MNWM;
    ...
    public override void OnStartLocalPlayer()
    {
        ...
        {
            GameObject inputObj = GameObject.Find("チャット欄"); // ヒエラルキー上の名前
            if (inputObj)
                m_ChatInputField = inputObj.GetComponent<InputField>();
        }

        //ネットワークマネージャーから名前を取得
        GameObject NTM = GameObject.Find("NetworkManager");
        if (NTM)
            m_MNWM = NTM.GetComponent<MirrorNetWorkManage>();

        if (m_MNWM != null)
        {
            m_PlayerName = m_MNWM.m_PlayerName;
        }
        else
        {
            Debug.Log("プレイヤーネームが存在しない、もしくは上手く同期できていません");
        }
    ...
    public void OnSendButton()
    {
        ...
        {
            CmdSendMessage(m_PlayerName + ": " + m_ChatInputField.text, null, null, -1);
            m_ChatInputField.text = ""; // 入力欄をクリア
        }
    }
